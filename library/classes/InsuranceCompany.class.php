...
class InsuranceCompany extends ORDataObject{
	...
	*/
	...
	function InsuranceCompany($id = "", $prefix = "")	{
		...
		$this->_table2 = "insurance_verification";
		...
		if ($id != "") {
			$this->set_payer_settings($id,'availity');
			$this->populate();
		}
	}
	...
	function set_payer_settings($id,$authority) {
		$this->payer_settings = $this->payer_settings_array($id,$authority);
	}
	...
	function persist() {
		parent::persist();
		$this->payer_settings_save($this->id,'availity');
	...
	function payer_settings_array($id,$authority) {
		$ins_verification = array();
		$query_payer = "
			SELECT 
				verification_type,
				verification_value
			...
			WHERE
				...
				insurance_id = '$id'
		";
		$query_res_payer = sqlQ($query_payer);
		while ($res_payer = mysql_fetch_array($query_res_payer) ) {
			$ins_verification[$res_payer['verification_type']] = $res_payer['verification_value'];
		}
		return $ins_verification;
	}
	
	function payer_settings_save($id,$authority) {
			...
			if($name_parts[0] == $authority){
				$query_update_payer = "
					REPLACE INTO
						" . $this->_table2 ."
					SET
						verification_type = '$name_parts[2]',
						verification_value = '$val',
						verification_authority = '$authority',
						insurance_id = '$id'
				";
				sqlQuery($query_update_payer);
			}
		}
	}
