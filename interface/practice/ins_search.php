  ...
  if ($ins_id) {
   ...
   ")");
		...
		foreach($_POST as $key=> $val){
			$name_parts = explode('_',$key,3);
			if($name_parts[0] == $authority){
				if($field_placeholders){
					$field_placeholders .= ",";
				}
				$field_placeholders .= "(?,?,?,?)";
				array_push($field_values,$ins_id,$authority,$name_parts[2],$val);
			}
		}
		if($field_placeholders && count($field_values)){
			$query_insert_payer = "INSERT INTO insurance_verification (insurance_id, verification_authority, verification_type, verification_value) VALUES ".$field_placeholders;
			sqlStatement($query_insert_payer,$field_values);
		}
  }
...
<table style='border:1px solid;padding:10px;' >
	...
	<tr>
		<td>
			<b><?php xl('Payer ID for xml','e');?>:</b>
		</td>
		<td>
			<input type="text" name="availity_payername_xml" id="availity_payername_xml" >
		</td>
	</tr>
	<tr>
		<td>
			<b><?php xl('Payer ID for eligibility','e');?>:</b>
		</td>
		<td>
			<input type="text" name="availity_payername_eligibility" id="availity_payername_eligibility" >
		</td>
	</tr>
	<tr>
		<td>
			<b><?php xl('Payer ID for claims status','e');?>:</b>
		</td>
		<td>
			<input type="text" name="availity_payername_claims_status" id="availity_payername_claims_status" >
		</td>
	</tr>
	<tr>
		<td>
			<b><?php xl('Payer ID for Authorization & Referral','e');?>:</b>
		</td>
		<td>
			<input type="text" name="availity_payername_authorization" id="availity_payername_authorization" >
		</td>
	</tr>
	<tr>
		<td>
			<b><?php xl('Payer ID for Health Care Claims','e');?>:</b>
		</td>
		<td>
			<input type="text" name="availity_payername_healthcare_claims" id="availity_payername_healthcare_claims" >
		</td>
	</tr>
	<tr>
		<td>
			<b><?php xl('Payer ID for Health Care Encounters','e');?>:</b>
		</td>
		<td>
			<input type="text" name="availity_payername_healthcare_encounters" id="availity_payername_healthcare_encounters" >
		</td>
	</tr>
</table>
