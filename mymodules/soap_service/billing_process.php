...
include_once(dirname(__FILE__)."/../../library/patient.inc");
include_once(dirname(__FILE__)."/../../library/billrep.inc");
include_once(dirname(__FILE__)."/../../library/billing.inc");
include_once(dirname(__FILE__)."/../../library/gen_x12_837.inc.php");
include_once(dirname(__FILE__)."/../../library/gen_hcfa_1500.inc.php");
include_once(dirname(__FILE__) . "/../../library/classes/WSClaim.class.php");
require_once(dirname(__FILE__)."/../../library/classes/class.ezpdf.php");
...
$bill_info = array();
global $bat_gscount, $bat_yymmdd, $bat_yyyymmdd, $bat_hhmm, $bat_icn,$bat_type,$bat_recvid,$bat_content,$bat_stcount,$bat_time;
...
$bat_filename = date("Y-m-d-Hi", $bat_time) . "-".$sqlconf['dbase'].".";
...
function process_form($ar,$pdf) {
  ...
  foreach ($ar['claims'] as $claimid => $claim_array) {
    ...
    if (isset($claim_array['bill'])) {
      ...
      //} else if ((isset($ar['bn_process_hcfa']) || isset($ar['bn_hcfa_txt_file'])) && $GLOBALS['zh_healthcare'] == true) {
       // $tmp = updateClaim(true, $patient_id, $encounter, $payer_id, $payer_type, 0, 1, '', 'hcfa');
	  } else if ((isset($ar['bn_process_hcfa']) || isset($ar['bn_hcfa_txt_file']))) {// && $GLOBALS['zh_healthcare'] != true
