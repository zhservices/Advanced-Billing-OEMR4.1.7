...
//actual execution is done in the select_query function in Server_side
  ...
  public function query_formation($data){
        ...
        switch($data[0]){
            ...
            case 'A5':
            include_once(dirname(__FILE__).'/../../library/formdata.inc.php');
            $enc_set_array=array();
            $enc_set_array[]=$data[1][3];
            ...
            $query="select concat(encounter,code,modifier) as pecm,encounter,code,session_id,
	    ...
	    case 'A14':
		  ...
		  $query="SELECT b.pid,b.encounter,b.date AS bdate, b.code_type, b.code, b.modifier, b.code_text, b.fee , b.units, b.justify
			FROM (form_encounter AS f,patient_data AS p)
			LEFT JOIN billing AS b ON b.pid=f.pid AND b.encounter=f.encounter AND b.activity=1 AND b.fee != 0.00
			WHERE ($where) AND p.pid = f.pid
			ORDER BY f.pid,f.billing_facility, f.date DESC, f.encounter DESC";
		  return array($query,$wherearray);
		  break;
	    ...
	    // Entries pending  for approval for Existing Patient and New Patient.
           ...
           //for building patient Demo   Employer Data
            ...
            case 'P20':	
            $query="select count(*) AS count from patient_data where pubpid = ? and pid !=?";
            return array($query,$data[1]);
            break;
        
			//getting DOB and SSN for verifying the duplicate patient existance
            case 'P21':	
            $query="select  ss,DOB  from patient_data where DOB=? and pid !=?  ";
            return array($query,$data[1]);
            break;
      
	    case 'P22':
						$query = "select fname,mname,lname,pid,DOB FROM patient_data WHERE lname LIKE REPLACE('".$data[1]."%',' ','%') OR fname LIKE REPLACE('".$data[1]."%',' ','%') ORDER BY id";
						if(preg_match("/^\d+$/",$data[1])){
							$query = "select fname,mname,lname,pid,DOB FROM patient_data WHERE pid='".$data[1]."'";
							if(preg_match("/(\d{2})(\d{2})(\d{4})/",$data[1],$res)){
								$query = "select fname,mname,lname,pid,DOB FROM patient_data WHERE pid='".$data[1]."' OR dob = CONCAT($res[3],'-',$res[1],'-',$res[2]) ORDER BY id";
							}
						}elseif(preg_match("/(-|\/)/",$data[1],$res)){
							if(preg_match("/(\d{2})(\d{2})(\d{4})/",str_replace($res[0],'',$data[1]),$res)){
								$query = "select fname,mname,lname,pid,DOB FROM patient_data WHERE dob = CONCAT($res[3],'-',$res[1],'-',$res[2]) ORDER BY id";
							}
						}
	    ...
	    case 'REMARKS':
	    $query = "SELECT * FROM arr_codes";
	    return array($query);
            break;
      
	    case 'STATUS':
	    $query = "SELECT DISTINCT(ac_status) FROM arr_codes";
	    return array($query);
            break;
      
	    case 'CATEGORY':
	    $query = "SELECT * FROM customlists AS cl WHERE cl.cl_list_type=20";
	    return array($query);
	    break;
      
	    case 'TDR'://todaydenialremarks
	    $query = "SELECT * FROM arr_codes";
	    return array($query);
	    break;
      
	    case 'USERAUTH':
	    $query = "SELECT authorized,see_auth FROM users WHERE username=?";
	    return array($query,array($_SESSION['authUser']));
            break;
        }
    }
}
