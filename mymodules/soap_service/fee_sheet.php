...
class FeeSheet Extends Billing{
  ...
  public function fee_sheet_update7($data){
    if($this->valid($data[0])){
      list($credentials,$authId,$reference,$check_date,$copay_val,$payment_method,$adj_code,$billFac,$SID) = $data;
  ...
  public function fee_sheet_update8($data){
    if($this->valid($data[0])){
      list($credentials,$cod,$mod,$authId,$copay_val,$account_code,$SID) = $data;
      sqlStatement("UPDATE ar_activity SET code='$cod', modifier='$mod', post_user='$authId', post_time=now(), pay_amount='$copay_val', modified_time=now() WHERE account_code='$account_code' AND session_id='$SID'");
    }
    else{
  ...
  public function fee_sheet_insert4($data){
    if($this->valid($data[0])){
      list($credentials,$payerID,$authId,$reference,$check_date,$pay_total,$global_amount,$insurance,$capitation,$pID,$payment_method,$adj_code,$billFac) = $data;
  ...
  public function fee_sheet_insert5($data){
    if($this->valid($data[0])){
      list($credentials,$pID,$encounterID,$code,$modifier,$payer_type,$authId,$session_id,$pay_amount,$account_code) = $data;
      return idSqlStatement("INSERT INTO ar_activity (pid,encounter,code,modifier,payer_type,post_time,post_user,session_id,pay_amount,account_code)".
        " VALUES ('$pID','$encounterID','$code','$modifier','$payer_type',now(),'$authId','$session_id','$pay_amount','$account_code')"
      );
    }
    else{
