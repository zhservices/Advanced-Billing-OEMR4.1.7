							...
							while ($RowSearch = sqlFetchArray($ResultSearch))
							 {
								...
								if($BillingId!=$Id)//multiple cpt in single encounter
								...
								else
								 {
									...
									$DedAmountDB[$DedAmountCount] = $DedAmountDB[$DedAmountCount]>0 ? $DedAmountDB[$DedAmountCount] : $DeductibleDB;
									...
									if($Ins==1)
									 {
									       for($j=1;$j<=$AdjAmountCount;$j++){
										$AllowedDB[$j]=number_format($Fee-$AdjAmountDB[$j],2);
									       }
									 }
									else
									 {
									...
									$AllowedDB[1]=$AllowedDB[1] == 0 ? '' : $AllowedDB[1];
								...
								elseif($Ins==0)
								 ...
								 for($k=1;$k<=$AdjAmountCount;$k++){
								 $adjamttot=$adjamttot+$AdjAmountDB[$k];
								 }
								 $deductibletot=$deductibletot+array_sum($DedAmountDB);
								 $takebacktot=$takebacktot+$TakebackDB;
								 $allowedtot=$allowedtot+array_sum($AllowedDB);
						  ?>
						  <tr class="text"  bgcolor='<?php echo $bgcolor; ?>' id="trCharges<?php echo $CountIndex; ?>">
							...
							<td class="<?php echo $StringClass; ?>" >
							 ...
							 <table border="0" cellpadding="0" cellspacing="0" align="center" id="AdjustmentTable<?php echo $CountIndex;?>">
							  <?php
							  $count = $DedAmountCount>$AdjAmountCount ? $DedAmountCount : $AdjAmountCount;
							  for($k=1;$k<=$count;$k++){
							  ?>
							  <tr class="text">
							   <td>
							 ...
							 ?>
							 <input  name="AdjAmount<?php echo $CountIndex; ?>-Sub1"  onKeyDown="PreventIt(event)"   autocomplete="off"  id="AdjAmount<?php echo $CountIndex; ?>-Sub1"  value="<?php echo htmlspecialchars($AdjAmountDB[1]); ?>"   onChange="ValidateNumeric(this);ScreenAdjustment(this,<?php echo $CountIndex; ?>,1);UpdateTotalValues(1,<?php echo $TotalRows; ?>,'AdjAmount','AdjAmounttotal');RestoreValues(<?php echo $CountIndex; ?>)"  type="text"   style="width:40px;text-align:right; font-size:12px" />
							 <?php
							 }
							 ?>
							 <input type="hidden" name="subcount<?php echo $CountIndex; ?>" id="subcount<?php echo $CountIndex; ?>" value="<?php echo $count;?>" />
							</td>
							<td class="<?php echo $StringClass; ?>" >
							 <?php
							 if($GLOBALS['msp_capturing_payment_screen']){
							 ...
							 ?>
							 <input  name="Deductible<?php echo $CountIndex; ?>-Sub1"  id="Deductible<?php echo $CountIndex; ?>-Sub1"  onKeyDown="PreventIt(event)"  onChange="ValidateNumeric(this);UpdateTotalValues(1,<?php echo $TotalRows; ?>,'Deductible','deductibletotal');"  value="<?php echo htmlspecialchars($DedAmountDB[1]); ?>"   autocomplete="off"   type="text"   style="width:40px;text-align:right; font-size:12px" />
							 <?php
							 }
							 ?>
							</td>
