...
function waitandappend(ele,val){
  sel_list[count] = new Array();
  sel_list[count][0] = ele;
  sel_list[count][1] = val;
  count++;
}
function appendoption(sel){
  for(i=0;i<sel.length;i++){
    document.getElementById(sel[i][0]).add(sel[i][1]);
  }
}
...
function RestoreValues(CountIndex)
 {//old remainder is restored back
   SubCount = document.getElementById('subcount'+CountIndex).value;
   BlankValue = 0;
   for(i=1;i<=SubCount;i++){
   if(document.getElementById('Allowed'+CountIndex).value*1==0 && document.getElementById('Payment'+CountIndex).value*1==0 && document.getElementById('AdjAmount'+CountIndex+'-Sub'+i).value*1==0 && document.getElementById('Takeback'+CountIndex).value*1==0)
    {
	 BlankValue++;
    }
   }
   if(BlankValue==SubCount){
  ...
  if(document.getElementById('Allowed'+CountIndex).id==PassedObject.id)
   {
	  if(document.getElementById('HiddenIns'+CountIndex).value==1)
	   {
		  document.getElementById('AdjAmount'+CountIndex+'-Sub1').value=Math.round((ChargeAmount-Allowed)*100)/100;
	   }
	  else
	   {
		  document.getElementById('AdjAmount'+CountIndex+'-Sub1').value=Math.round((Remainder-Allowed)*100)/100;
	   }
   }
  AdjustmentAmount=0;
  for(SubIndex=1;SubIndex<=SubCount;SubIndex++){
  AdjustmentAmount+=document.getElementById('AdjAmount'+CountIndex+'-Sub'+SubIndex).value*1;
  }
	...
	for(RowCount=CountIndexAbove+1;;RowCount++)
	 {
	  if(!document.getElementById('Payment'+RowCount))
	   break;
	  SubCount = document.getElementById('subcount'+RowCount).value;
	  for(SubIndex=1;SubIndex<=SubCount;SubIndex++){
	  if(!document.getElementById('Payment'+RowCount))
	   break;
	  else if(document.getElementById('Allowed'+RowCount).value=='' && document.getElementById('Payment'+RowCount).value=='' && document.getElementById('AdjAmount'+RowCount+'-Sub'+SubIndex).value=='' && document.getElementById('Deductible'+RowCount+'-Sub'+SubIndex).value=='' && document.getElementById('Takeback'+RowCount).value=='' && document.getElementById('FollowUp'+RowCount).checked==false)
	  {
	  }
	 else
	   {
...
{//Used in totaling the columns.
 var paymenttot=0;
	 if(count > 0)
	 {
	   ...
	   {
	       if(Payment!='AdjAmount' && Payment!='Deductible'){
		    ...
		    subcount = document.getElementById('subcount'+i).value;
		    for(j=1;j<=subcount;j++){
			 if(document.getElementById(Payment+i+'-Sub'+j))
			 {
				   paymenttot=paymenttot+document.getElementById(Payment+i+'-Sub'+j).value*1;
			 }
		    }
	       }
	   }
...
function add_dynamic_row(id,start,total){
     document.getElementById('subcount'+id).value = parseInt(document.getElementById('subcount'+id).value)+1;
     SubCountVal = document.getElementById('subcount'+id).value;
     TableId = document.getElementById('AdjustmentTable'+id);
     rowCount = TableId.rows.length;
     row = TableId.insertRow(rowCount);
     cell1 = row.insertCell(0);
     cell1.setAttribute= ('class','top right');
     cell1.className = 'top right';
     element1 = document.createElement("input");
     element1.type = "text";
     element1.setAttribute('name','AdjAmount'+id+"-Sub"+SubCountVal);
     element1.setAttribute('id','AdjAmount'+id+"-Sub"+SubCountVal);
     element1.style.width = '40px';
     element1.style.fontSize = '12px';
     element1.setAttribute('style','width:40px;font-size:12px;text-align:right');
     element1.setAttribute('onchange','UpdateTotalValues('+start+','+total+',\'AdjAmount\',\'initialAdjAmounttotal\');RestoreValues('+id+');ScreenAdjustment(this,'+id+')');
     cell1.appendChild(element1);
     cell1 = row.insertCell(1);
     cell1.setAttribute= ('class','top right');
     cell1.className = 'top right';
     element1 = document.createElement("select");
     element1.setAttribute('name','AdjGRP'+id+"-Sub"+SubCountVal);
     element1.setAttribute('id','AdjGRP'+id+"-Sub"+SubCountVal);
     element1.style.width = '43px';
     element1.style.fontSize = '12px';
     element1.style.align = 'right';
     element1.setAttribute('style','width:40px;font-size:12px;text-align:right');
     for(i=0;i<document.getElementById('AdjGRP'+id+'-Sub1').length;i++){
      var valuu_option=document.createElement('option');
      valuu_option.text='';
      valuu_option.value='';
      valuu_option.text=document.getElementById('AdjGRP'+id+'-Sub1').options[i].text;
      valuu_option.value=document.getElementById('AdjGRP'+id+'-Sub1').options[i].value;
      try{
      element1.add(valuu_option,null);
      }
      catch(e){
	  waitandappend('AdjGRP'+id+'-Sub'+SubCountVal,valuu_option);
      }
     }
     cell1.appendChild(element1);
     cell1 = row.insertCell(2);
     cell1.setAttribute= ('class','top');
     cell1.className = 'top';
     element1 = document.createElement("input");
     element1.type = "text";
     element1.setAttribute('name','AdjRSN'+id+'-Sub'+SubCountVal);
     ...
     TableId = document.getElementById('DeductibleTable'+id);
     rowCount = TableId.rows.length;
     row = TableId.insertRow(rowCount);
     cell1 = row.insertCell(0);
     cell1.setAttribute= ('class','top right');
     ...
     element1 = document.createElement("input");
     element1.type = "text";
     element1.setAttribute('name','Deductible'+id+"-Sub"+SubCountVal);
     element1.setAttribute('id','Deductible'+id+"-Sub"+SubCountVal);
     element1.style.width = '40px';
     element1.style.fontSize = '12px';
     element1.style.align = 'right';
     element1.style.textAlign = 'right';
     element1.setAttribute('onChange','UpdateTotalValues('+start+','+total+',\'Deductible\',\'initialdeductibletotal\');RestoreValues('+id+');ScreenAdjustment(this,'+id+')');
     cell1.appendChild(element1);
     cell1 = row.insertCell(1);
     cell1.setAttribute= ('class','top right');
     cell1.className = 'top right';
     element1 = document.createElement("select");
     element1.setAttribute('name','DedGRP'+id+"-Sub"+SubCountVal);
     element1.setAttribute('id','DedGRP'+id+"-Sub"+SubCountVal);
     element1.style.width = '43px';
     element1.style.fontSize = '12px';
     element1.style.align = 'right';
     for(i=0;i<document.getElementById('DedGRP'+id+'-Sub1').length;i++){
      var valuu_option=document.createElement('option');
      valuu_option.text='';
      valuu_option.value='';
      valuu_option.text=document.getElementById('DedGRP'+id+'-Sub1').options[i].text;
      valuu_option.value=document.getElementById('DedGRP'+id+'-Sub1').options[i].value;
      try{
      element1.add(valuu_option,null);
      }
      catch(e){
	  waitandappend('DedGRP'+id+'-Sub'+SubCountVal,valuu_option);
      }
     }
     cell1.appendChild(element1);
     cell1 = row.insertCell(2);
     ...
     element1 = document.createElement("input");
     element1.type = "text";
     element1.setAttribute('name','DedRSN'+id+"-Sub"+SubCountVal);
     element1.style.width = '30px';
     element1.style.fontSize = '12px';
     element1.style.align = 'right';
     element1.style.textAlign = 'right';
