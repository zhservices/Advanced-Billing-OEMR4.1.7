  ...
  if ($_POST["mode"] == "new_payment" || $_POST["mode"] == "distribute")
   {
	...
	"', payment_screen = '"   . 0 .
	  ...
	  $cap_id = sqlInsert("insert into ar_session set "    .
	  	$QueryPart .
        "', user_id = '"     . trim($user_id                  )  .
        "', closed = '"      . trim($closed                   )  .
        "', reference = '"   . trim(formData('check_number'   )) .
		  ...
		  if($RowIndex>1)
		   {
			  if($Denial!='yes')
			   ...
			   {
				  $AffectedRowArray[$RowIndex-1]='no';
			   }
			  $Affected='no';
			  $Denial='';
		   }
...
<?php require_once($_SERVER['DOCUMENT_ROOT'] . "/modules/billingportal/reports/report.script.php");?>
...
function check_number_verify(val){
 if(val){
  $.ajax({
   type: "POST",
   url: "zh_ajax_code.php",
   dataType: "html",
   data: {
       action       : 'CheckNumberVerify',
       check_number : val,
   },
   success: function(thedata){
       if(thedata=='duplicate'){
	$('#check_number').css({"background-color": "red"});
	$('#check_number').css({"color": "white"});
       }
       else{
	$('#check_number').css({"background-color": "white"});
	$('#check_number').css({"color": "black"});
       }
   },
   error:function(){
       alert('Ajax Failed');
		  ...
		  <tr>
			<td colspan="13" align="left" >
				...
				<table border="0" cellspacing="0" cellpadding="0" width="267" align="center" id="AllocateButtons">
				  ...
				  <tr>
					<td width="100"><a href="#" onClick="javascript:return PostPayments();"  class="css_button" style="width:106px"><span><?php echo htmlspecialchars( xl('Post Payments'), ENT_QUOTES);?></span></a></td>
					<td width="107"><a href="#" onClick="javascript:return FinishPayments();"  class="css_button" style="width:106px"><span><?php echo htmlspecialchars( xl('Finish Payments'), ENT_QUOTES);?></span></a></td>
					<td width="60"><a href="#"  onClick="CancelDistribute()" class="css_button" style="width:106px"><span><?php echo htmlspecialchars( xl('Cancel'), ENT_QUOTES);?></span></a></td>
				  </tr>
				</table>
				<?php
				 }//if($CountIndexBelow>0)
