...
{//Used in the cancel button.Helpful while cancelling the distribution.
    if(confirm("<?php echo htmlspecialchars( 'Would you like to Cancel Distribution for this Patient?', ENT_QUOTES) ?>"))
    {
...
{//Used in saving the allocation
    ...
    {
        alert("<?php echo htmlspecialchars( 'Fill the Row.', ENT_QUOTES) ?>")
        return false;
    }
    ...
    if(PostValue==1)
    {
        alert("<?php echo htmlspecialchars( 'Cannot Post Payments.Undistributed is Negative.', ENT_QUOTES) ?>")
        return false;
    }
    if(confirm("<?php echo htmlspecialchars( 'Would you like to Post Payments?', ENT_QUOTES) ?>"))
    {
...
//After this is pressed a confirmation screen comes,where you can edit if needed.
    ...
    if(PostValue==2)
    {
        if(confirm("<?php echo htmlspecialchars( 'Would you like to Post and Finish Payments?', ENT_QUOTES) ?>"))
        {
            UnappliedAmount=document.getElementById('TdUnappliedAmount').innerHTML*1;
            if(confirm("<?php echo htmlspecialchars( 'Undistributed is', ENT_QUOTES) ?>" + ' ' + UnappliedAmount +  '.' + "<?php echo htmlspecialchars('\n');echo htmlspecialchars( 'Would you like the balance amount to apply to Global Account?', ENT_QUOTES) ?>"))
            {
...
{//Displays message after saving to master table.
    ...
    else if(after_value=='new_payment')
    {
        ...
        if(confirm("<?php echo htmlspecialchars( 'Successfully Saved.Would you like to Allocate?', ENT_QUOTES) ?>"))
        {
            if(document.getElementById('TablePatientPortion'))
            {
